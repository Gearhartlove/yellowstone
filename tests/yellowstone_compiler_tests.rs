...
fn compiler_unary() {
    ...
    assert_eq!(Ok(result), Some(Value::number_value(2.)));
}
...
fn compiler_binary() {
    ...
    let source = "1 - 2";
    let result = run_code(&mut vm, source);
    assert_eq!(Ok(result), Some(Value::number_value(-1.)));

    let source = "3 / 2";
    let result = run_code(&mut vm, source);
    assert_eq!(Ok(result), Some(Value::number_value(1.5)));

    let source = "2 * 2";
    let result = run_code(&mut vm, source);
    assert_eq!(Ok(result), Some(Value::number_value(4.)));
}
...
fn compiler_number() {
    ...
    let source = "2022";
    let result = run_code(&mut vm, source);
    assert_eq!(Ok(result), Some(Value::number_value(2022.)));
}
...
#[test]
fn compiler_assert_test() {
    let mut vm = VM::default();
    let source = "assert(true, false);";
    let result = run_code(&mut vm, source);
    assert_eq!(result, Err(InterpretError::RUNTIME_ASSERT_ERROR));
}
...
pub fn run_code<T: ToString + Display>(vm: &mut VM, code: T) -> Result<Option<Value>, InterpretError> {
